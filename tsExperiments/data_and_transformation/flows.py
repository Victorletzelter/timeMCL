# This file was adpated from https://github.com/zalandoresearch/pytorch-ts/blob/master/pts/modules/flows.py
# under MIT License.
    ...
    def inverse(self, u, cond):
        ...
        if self.scale is not None:
            if not isinstance(self.scale, dict):
                ...
                x *= self.scale["scale"]
                log_abs_det_jacobian += torch.log(torch.abs(self.scale["scale"]))
            elif len(self.scale) == 2:
                mean = self.scale["mean"]
                std = self.scale["std"]
                x = x * std + mean
                log_abs_det_jacobian += torch.log(torch.abs(std))
            else:
                raise ValueError(
                    "Scale must be a scalar or a dictionary with 'mean' and (optionally) 'std' key"
                )
