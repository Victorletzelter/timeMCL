# This code was adapted from https://github.com/zalandoresearch/pytorch-ts/blob/master/pts/model/deepvar/deepvar_network.py
# under MIT License
...
class deepVAR(nn.Module):
    ...
    @property
    def distr_output(self):
        if self.dist_type == "LowRankMultivariateNormalOutput":
            return LowRankMultivariateNormalOutput(**self.dist_params)
        else:
            raise ValueError(f"Distribution type {self.dist_type} not supported")
    ...
    def distr(
        ...
        if type(scale) == dict:
            if len(scale) == 1:
                ...
                mean = scale["mean"]
                std = scale["std"]
                distr = self.distr_output.distribution(distr_args, loc=mean, scale=std)
            else:
    ...
    def loss(
        ...
        distr, distr_args = self.distr(rnn_outputs=rnn_outputs, scale=scale_params)
