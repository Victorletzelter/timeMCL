...
# Adapted from https://github.com/ServiceNow/TACTiS/blob/tactis-2/tactis/model/tactis.py
...
class TACTiS(nn.Module):
    ...
    def __init__(
        ...
        if not self.skip_copula:
            ...
            copula_decoder["attentional_copula"][
                "input_dim"
            ] = self.copula_encoder_embedding_dim
        ...
        if positional_encoding is not None:
            self.flow_time_encoding = PositionalEncoding(
                self.flow_encoder_embedding_dim, **positional_encoding
            )
            if not self.skip_copula:
                self.copula_time_encoding = PositionalEncoding(
                    self.copula_encoder_embedding_dim, **positional_encoding
                )
        else:
        ...
        for i in range(self.flow_input_encoder_layers):
            if i == 0:
                flow_elayers.append(
                    nn.Linear(
                        self.flow_series_embedding_dim + 2,
                        self.flow_encoder_embedding_dim,
                    )
                )
            else:
                flow_elayers.append(
                    nn.Linear(
                        self.flow_encoder_embedding_dim, self.flow_encoder_embedding_dim
                    )
                )
            ...
            for i in range(self.copula_input_encoder_layers):
                if i == 0:
                    copula_elayers.append(
                        nn.Linear(
                            self.copula_series_embedding_dim + 2,
                            self.copula_encoder_embedding_dim,
                        )
