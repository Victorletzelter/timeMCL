...
class Evaluator:
    ...
    def get_metrics_per_ts(
        ...
        if self.custom_eval_fn is not None:
            for k, (eval_fn, _, fcst_type) in self.custom_eval_fn.items():
                if fcst_type == "mean":
                    ...
                    target_fcst = forecast  # we keep all the forecasts.
        ...
        # here we can compute for each of the hypothesis (the N predictions) the MSE.
        rmse_values = np.mean(
            (forecast.samples - pred_target.data) ** 2, axis=1
        )  # shape Nb_samples
        for i, mse in enumerate(rmse_values):
            key = f"sample_{i}"
            val = {key: mse}
            metrics.update(val)
    ...
    def get_aggregate_metrics(
        ...
        # aggregation for the samples - we're not going to aggregate other the differents samples.
...
class MultivariateEvaluator(Evaluator):
    ...
    def __call__(
        ...
        cumulative_samples = None  # for storing the ts by ts metrics
        for dim in eval_dims:
            ...
            )  # here MSE and RMSE are the same for a given dim.

            # ---related to distorsion evaluation ---------------------------------------------
            sample_cols = [
                col for col in metrics_per_ts.columns if col.startswith("sample_")
            ]
            if sample_cols:
                samples = metrics_per_ts[sample_cols].copy()
                # accumulation on a dataframe
                if cumulative_samples is None:
                    cumulative_samples = samples
                else:
                    cumulative_samples = cumulative_samples.add(samples, fill_value=0)

                metrics_per_ts = metrics_per_ts.drop(columns=sample_cols)

            # here we can extract the distorsion/sample for metrics_per_ts and saving the
        ...
        )  # here we do a MEAN of MSE on the differents dims. When we sum for distorsion.
        ...
        hyp_rmse = np.sqrt(cumulative_samples)  # shape (N_windows, N_hypotheses)
        min_per_row = hyp_rmse.min(axis=1)  # the minimum per row
        distorsion = np.mean(min_per_row)  # the distorsion ! for the global ts

        return all_agg_metrics, all_metrics_per_ts, distorsion
