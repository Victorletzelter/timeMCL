...
from tsExperiments.stock_market_data.config import (
    BINANCE_BASE_URL,
    TIME_ZONE_BERLIN,
    TIME_ZONE_JAKARTA,
    TIME_ZONE_PARIS,
...
from tsExperiments.stock_market_data.data_processors._base import _Base, calc_time_zone
...
class Yahoofinance(_Base):
    ...
    def download_data(
        ...
        load_from_csv_path: str = None,
    ):
        if load_from_csv:
            if not os.path.exists(load_from_csv_path):
                raise FileNotFoundError(f"CSV file not found at {load_from_csv_path}")
            self.dataframe = pd.read_csv(load_from_csv_path)
            if 'date' in self.dataframe.columns:
                self.dataframe['date'] = pd.to_datetime(self.dataframe['date'])
            self._standardize_columns()
            print(f"Data loaded from {load_from_csv_path}")
            print("Shape of DataFrame: ", self.dataframe.shape)
            return
